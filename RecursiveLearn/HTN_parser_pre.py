...
import copy
class PDDLParser:
  def __init__(self, pddl_content, enforce_remove_methods = False):
    self.remove_methods = enforce_remove_methods
    ...
    if len(method1["subtasks"]) != len(method2["subtasks"]):
      ...
      for prec in method1["precondition"]:
        ...
        if not prec_parameters:
          # we don't need to care about this precondition
          continue
        for prec_parameter in prec_parameters:
          if prec_parameter not in parameter1:
            # there is an irrelevant parameter in the precondition
            flag = True
          else:
      ...
      for j in range(i+1, len(self.parsed_methods)):
        ...
        if self.parsed_methods[i]["head"] == self.parsed_methods[j]["head"]:
          if self.have_same_subtasks(self.deep_copy(self.parsed_methods[i]), self.deep_copy(self.parsed_methods[j])):
            ...
            # success = True
            if self.remove_methods and success:
  ...
  def merge_methods(self, method1, method2):
    ...
    method2["precondition"] = self.get_common_precondition(method1, method2)
    ...
    if not flag:
      ...
      if __name__ == "__main__":
        print("Successfully Merged Two Methods.")
        self.write_method(method2)
    return True
...
if __name__ == "__main__":
  ...
  with open(file, "r") as pddl_file:
    ...
    # Parse the PDDL file
    parser = PDDLParser(pddl_content, enforce_remove_methods = remove_methods)
