...
class ConverterToPDDLString(walkers.DagWalker):
    ...
    def convert(self, expression):
        ...
        # print(self.simplifier.simplify(expression))
...
class PDDLWriter:
    ...
    def _write_problem(self, out: IO[str], domain):
        if self.problem.name is None:
            name = "pddl"
        else:
            name = _get_pddl_name(self.problem)
        ...
        if len(self.problem.user_types) > 0:
            ...
            for t in self.problem.user_types:
                ...
                if len(objects) > 0:
                    for o in objects:
                        out.write(
                            f"\n   {self._get_mangled_name(o)} - {self._get_mangled_name(t)}"
                        )
                    # out.write(
        ...
        for f, v in self.problem.initial_values.items():
            # print(f"initial value: {f} = {v}, {converter.convert(f)}")
            if v.is_true():
                # convert at_lander(general, waypoint0) to (at_lander general waypoint0) using regular expression
                # input_str = "at_lander(general, waypoint0, 1, 2, 3)"
                output_str = re.sub(r"([\w,-]+)\(([\w,\s]+)\)", r"(\1 \2)", str(f)).replace(",", "")
                # print(output_str)  # (at_lander general waypoint0 1 2 3)
                # out.write(f"\n\t {converter.convert(f)}")
                out.write(f"\n\t {output_str}")
            elif v.is_false():
                pass
        ...
        # arbitrarily choose one goal using random.choice
        out.write(f"\t(:goal\n\t\t(and\n\t{random.choice(goals_str)}\n\t\t)\n\t)\n")
    ...
    def write_problem(self, filename: str, domain = None):
        """Dumps to file the `PDDL` problem."""
        with open(filename, "w") as f:
            self._write_problem(f, domain)
