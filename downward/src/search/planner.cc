...
int main(int argc, const char **argv) {
    ...
    if (static_cast<string>(argv[1]) != "--help") {
        ...
        utils::g_log << "getting initial state..." << endl;
        State initial_state = task_proxy.get_initial_state();
        utils::g_log << "Number of vars:" << initial_state.size() << endl;
        // utils::g_log << "Number of vars:" << initial_state.num_variables << endl;
        for (std::size_t i = 0; i < initial_state.size(); i++) {
            FactProxy fact = initial_state[i];
