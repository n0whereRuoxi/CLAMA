...
bool SearchAlgorithm::check_goal_and_set_plan(const State &state) {
    if (task_properties::is_goal_state(task_proxy, state)) {
        ...
        set_plan(plan);        
        string filename = "current_state.1";
        utils::g_log << "Printing and saving the current state to " << filename << endl;
        ofstream outfile(filename);
        if (outfile.rdstate() & std::ofstream::failbit) {
            std::cerr << "Failed to open plan file: " << filename << endl;
            utils::exit_with(utils::ExitCode::SEARCH_INPUT_ERROR);
        }
        for (std::size_t i = 0; i < state.size(); i++) {
            FactProxy fact = state[i];
            std::string fact_name = fact.get_name();
            utils::g_log << "Fact name:" << fact_name << endl;
            outfile << fact_name << endl;
        }
        return true;
    }
    return false;
