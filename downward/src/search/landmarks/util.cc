...
void dump_landmark_graph(
    ...
    // dump node to file
    static void dump_node(const TaskProxy &task_proxy, const LandmarkNode &node, ofstream &file) {
        file << "  lm" << node.get_id() << " [label=\"";
        bool first = true;
        const Landmark &landmark = node.get_landmark();
        for (FactPair fact : landmark.facts) {
            if (!first) {
            ...
            first = false;
            VariableProxy var = task_proxy.get_variables()[fact.var];
            file << var.get_fact(fact.value).get_name();
        }
        file << "\"";
        if (landmark.is_true_in_state(task_proxy.get_initial_state())) {
            file << ", style=bold";
    ...
    // dump edge to file
    static void dump_edge(int from, int to, EdgeType edge, ofstream &file) {
        file << "      lm" << from << " -> lm" << to << " [label=";
        switch (edge) {
        case EdgeType::NECESSARY:
            file << "\"nec\"";
            break;
    ...
    // a function like dump_landmark_graph but saves the graph in a file
    void save_landmark_graph(const TaskProxy &task_proxy, const LandmarkGraph &graph, const string &filename) {
        utils::g_log << "Saving landmark graph: " << endl;
        ofstream file;
        file.open(filename);
        file << "digraph G {\n";
        for (const unique_ptr<LandmarkNode> &node : graph.get_nodes()) {
            dump_node(task_proxy, *node, file);
            for (const auto &child : node->children) {
                const LandmarkNode *child_node = child.first;
                const EdgeType &edge = child.second;
                dump_edge(node->get_id(), child_node->get_id(), edge, file);
            }
        }
        file << "}" << endl;
        file.close();
