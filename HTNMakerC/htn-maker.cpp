...
HtnDomain * DoOneExperiment(std::string a, 
                            std::string b,
                            std::string c,
                            std::string d,
                            std::string e,
...
int main( int argc, char * argv[] )
{
  ...
  std::string l_sCurriculumFile;
  std::string l_sProblemFile;
  std::string l_sSolutionFile;
  std::string l_sHtnDomainFile;
  std::string l_sTaskFile;
  std::string l_sOutputHtnDomainFile;
  ...
  TCLAP::UnlabeledValueArg<std::string> l_aCurriculumFile( "curriculum_file", "Path to the curriculum file.", true, "not_spec", "curriculum_file", l_cCmd );
  ...
  TCLAP::UnlabeledValueArg<std::string> l_aOutputHtnDomainFile( "output_htn_domain_file", "Path to the Output HTN domain file.", true, "not_spec", "output_htn_domain_file", l_cCmd );
  TCLAP::UnlabeledValueArg<std::string> l_aTaskFile( "tasks_file", "Path to the tasks file.", false, "not_spec", "tasks_file", l_cCmd );
  ...
  l_sCurriculumFile = l_aCurriculumFile.getValue();
  l_sProblemFile = l_aProblemFile.getValue();
  l_sSolutionFile = l_aSolutionFile.getValue();
  l_sTaskFile = l_aTaskFile.getValue();
  l_sHtnDomainFile = l_aHtnDomainFile.getValue();
  l_sOutputHtnDomainFile = l_aOutputHtnDomainFile.getValue();
  ...
  try {
    std::cout << "Start HTNMakerC\n";
    HtnDomain * l_pHtnDomain = DoOneExperiment(l_sStripsDomainFile, l_sCurriculumFile, l_sProblemFile, l_sSolutionFile, l_sHtnDomainFile, l_sTaskFile);
    std::ofstream outfile(l_sOutputHtnDomainFile);
    outfile << l_pHtnDomain->ToPddl();
    int l_iNumMethods = l_pHtnDomain->GetNumMethods();
    std::ofstream l_fMethodsNumber;
    l_fMethodsNumber.open("./number.txt", std::ios_base::app);
    l_fMethodsNumber << l_iNumMethods << std::endl;
    l_fMethodsNumber.close();
...
HtnDomain * DoOneExperiment(std::string l_sStripsDomainFile, std::string l_sCurriculumFile, 
  std::string l_sProblemFile, std::string l_sSolutionFile, std::string l_sHtnDomainFile, std::string l_sTasksFile)
{

  std::tr1::shared_ptr< StripsDomain > l_pStripsDomain;
  std::tr1::shared_ptr< StripsProblem > l_pStripsProblem;
  std::vector<std::vector<std::string>> l_vCurriculum;
  AnnotatedPlan * l_pStripsPlan = 0;
  HtnTaskList * l_pHtnTaskList = 0;
  std::map<std::string, HtnTaskList*> l_mHTHTaskMap;
  ...
  try
  {
    ...
    // std::cout << "Reading htn domain file step II " << l_sHtnDomainFile << std::endl;
  ...
  if( g_iFlags & FLAG_ND_CHECKERS ){
    std::cout << "Make Trivial NdCheckers" << std::endl;
    MakeTrivialNdCheckers( l_pHtnDomain );
  }
  
  if (g_iFlags & FLAG_CURRICULUM) {

    std::cout << "Doing curriculum learning......" << std::endl;
    try
    {
      std::cout << "Reading curriculum file " << l_sCurriculumFile << std::endl;
      std::vector<std::string> row;
      std::string line, word;
      std::fstream l_fCrruculum(l_sCurriculumFile, std::ios::in);
      if(l_fCrruculum.is_open()){
        while(getline(l_fCrruculum, line)){
          row.clear();
          std::stringstream str(line);
          while(getline(str, word, ','))
            row.push_back(word);
          l_vCurriculum.push_back(row);
        }
      }
      else
        std::cout<<"Could not open the file\n";
      for(long unsigned int i=0;i<l_vCurriculum.size();i++){
        for(long unsigned int j=0;j<l_vCurriculum[i].size();j++){
          std::cout<<l_vCurriculum[i][j]<<" ";
        }
        std::cout<<"\n";
      }
    }
    ...
    {
      e.SetFileName( l_sCurriculumFile );
      throw e;
    }
    ...
    for(long unsigned int i=0;i<l_vCurriculum.size();i++){
      ...
      try
      {
        l_mHTHTaskMap.insert( {l_vCurriculum[i][2], new HtnTaskList( std::tr1::shared_ptr< HtnDomain >( new HtnDomain( *l_pHtnDomain ) ), 
                                                            ReadFile( l_sDir + "/task_" + l_vCurriculum[i][2] + ".pddl" ) ) } );
      }
      catch( FileReadException & e )
      {
        e.SetFileName(  "/task_" + l_vCurriculum[i][2] + ".pddl" );
        throw e;
      }
      // l_vTasks.push_back(l_vCurriculum[i][2])
      std::cout << "Done reading task file" << "/task_" + l_vCurriculum[i][2] + ".pddl" << std::endl;
    }
    ...
    std::cout << "Doing curriculum learning" << std::endl;
    for(long unsigned int i=0;i<l_vCurriculum.size();i++){
      // std::cout << "Current step: " << l_vCurriculum[i][0] << " " << l_vCurriculum[i][1] << std::endl;
      LearnMethodsWithCurriculumStep(std::stoi(l_vCurriculum[i][0]),
                                      std::stoi(l_vCurriculum[i][1]),
                                      l_pStripsPlan,
                                      l_mHTHTaskMap[l_vCurriculum[i][2]],
                                      l_pHtnDomain );
    }
  }

  else {
    std::cout << "Doing HTN learning without curriculum......" << std::endl;

    try
    {

    std::cout << "Reading task list..." << std::endl;
    ...
    if( g_iFlags & FLAG_SOUNDNESS_CHECK ){
      std::cout << "Make Soundness Check Methods" << std::endl;
      MakeSoundnessCheckMethods( l_pHtnTaskList,
              l_pHtnDomain );
    }

    std::cout << "Doing HTN regular learning" << std::endl;
    LearnMethods( l_pStripsPlan,
		l_pHtnTaskList,
		l_pHtnDomain );
  }

  std::cout << "HTN-Maker-C learned " << l_pHtnDomain->GetNumMethods() << " methods" << "\n";
