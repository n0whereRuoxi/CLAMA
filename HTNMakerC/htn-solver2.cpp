...
void DoOneExperiment(std::string l_sDomainFile, std::string l_sProblemFile, 
  std::ofstream &l_oFile, std::ofstream &l_oFileMeta, std::clock_t c_start);

int DoExperiments(std::string l_sDomainName, std::string l_sP, int n);
...
int main( int argc, char * argv[] )
{
  ...
  try{
    ...
    try
    {
      ...
      TCLAP::UnlabeledValueArg<std::string> l_aProblem( "problem", "Which problem?", true, "not_spec", "problem", l_cCmd );
      TCLAP::UnlabeledValueArg<int> l_aNumTrain( "number", "Number of training problems?", true, 0, "number", l_cCmd );
      TCLAP::SwitchArg l_aPrec( "y", "precondition", "Annotated tasks has precondition", l_cCmd, false );
      TCLAP::SwitchArg l_aGeneralized( "g", "generalized", "Generalized method extracted", l_cCmd, false );
      ...
      l_sProblemNum = l_aProblem.getValue();
      l_iNumTrain = l_aNumTrain.getValue();
      g_bGeneralized = l_aGeneralized.getValue();
    ...
    return DoExperiments(l_sDomainName, l_sProblemNum, l_iNumTrain);
...
int DoExperiments(std::string l_sDomainName, std::string l_sProblemNum, int l_sNumTrain)
{
  std::string l_sDomainFile;
  std::string l_sPlanMetaFile;
  std::string l_sPlanFile;
  if (g_bGeneralized) {
    l_sDomainFile = "./" + l_sDomainName + "/results/result_domain_htn_" + std::to_string(l_sNumTrain) + "_generalized.pddl" ;
    l_sPlanMetaFile  = "./" + l_sDomainName + "/results/plans" + "/planmeta_" + std::to_string(l_sNumTrain) + "_generalized.txt";
    l_sPlanFile = "./" + l_sDomainName + "/results/plans" + "/plan" + l_sProblemNum + "_generalized.plan";
    std::cout << "Output plan meta file is " << l_sPlanMetaFile << std::endl;
  }
  else {
    l_sDomainFile = "./" + l_sDomainName + "/results/result_domain_htn_" + std::to_string(l_sNumTrain) + ".pddl" ;
    l_sPlanMetaFile  = "./" + l_sDomainName + "/results/plans" + "/planmeta_" + std::to_string(l_sNumTrain) + ".txt";
    l_sPlanFile = "./" + l_sDomainName + "/results/plans" + "/plan" + l_sProblemNum + ".plan";
    std::cout << "Output plan meta file is " << l_sPlanMetaFile << std::endl;
  }
  std::string l_sProblemFile = "/scratch/zt1/project/nau-lab/user/rli12314/CurricuLAMA/experiments/" + l_sDomainName + "/htn_probs" + "/prob" + l_sProblemNum + "_htn.pddl";

  std::ofstream l_oPlanMeta;
  
  l_oPlanMeta.open(l_sPlanMetaFile, std::fstream::in | std::fstream::out | std::fstream::app);
  std::ofstream l_oPlan;
  std::cout << "writing plan to: " + l_sPlanFile << std::endl;
  l_oPlan.open(l_sPlanFile);
  std::clock_t c_start = std::clock();
  try
  {
    DoOneExperiment(l_sDomainFile, l_sProblemFile, l_oPlan, l_oPlanMeta, c_start);
  }
  ...
  std::clock_t c_start)
{
  ...
  std::cout << "DoOneExperiment " << std::endl;
      ...
      std::clock_t c_start)
{
  ...
  std::cout << "FindPlanOper for: " << p_pPartial->GetCTopTask()->GetName() << std::endl;
...
//    return false;
  ...
  std::cout << "Found Operator Index: " << l_iOperIndex << std::endl;
  ...
  if( l_iOperIndex != -1 )
  {
    ...
    std::cout << "l_OperSubs: " << l_OperSubs.ToStr();
    ...
    std::cout << "number of l_pAllOperSubs is " <<l_pAllOperSubs->size() << std::endl;
